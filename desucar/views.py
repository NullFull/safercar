...
def detail(request, maker_name, car_name, car_year, car_code):
    ...
    defects = {
        '리콜': {
            'code': 'recall',
            'desc': '리콜은 소비자의 안전을 위협하는 중대한 결함을 바로잡기 위해 시행합니다. 이곳에서는 국토교통부 자동차리콜센터(www.car.go.kr)와 환경부가 제공한 2000년 이후 리콜 자료 전체를 보여드립니다.',
            'items': [x for x in official_defects if x.kind == 'RC'],
        },
        '무상수리': {
            'code': 'freefix',
            'desc': '무상수리는 리콜 대상이 아닌 다양한 제작 결함을 바로잡기 위해 시행합니다. 이곳에서는 국토교통부 자동차리콜센터(www.car.go.kr)와 한국소비자원 소비자위해감시시스템(www.ciss.go.kr)에 등록된 2000년 이후 무상수리 자료 전체를 보여드립니다.',
            'items': [x for x in official_defects if x.kind == 'FF'],
        },
        '비공식 결함': {
            'code': 'unofficial',
            'desc': '정부와 제작사가 공식적으로 밝힌 리콜이나 무상수리 외에도 비공식적으로 처리되는 중요한 결함 정보가 많습니다. 이곳에서는 자동차 동호회, 인터넷 커뮤니티, 뉴스타파 제보 등을 통해 수집된 다양한 비공식 결함 정보를 알려드립니다.',
